...
(0, mocha_1.describe)('VersionManifest', () => {
    ...
    (0, mocha_1.describe)('updateContent', () => {
        ...
        (0, mocha_1.it)('updates versions.txt with an artifact id is another ones suffix with snapshot', async () => {
            const oldContent = (0, fs_1.readFileSync)((0, path_1.resolve)(fixturesPath, './versions-suffix-with-snapshot.txt'), 'utf8').replace(/\r\n/g, '\n');
            const versions = new Map();
            versions.set('google-cloud-admin', version_1.Version.parse('2.3.4'));
            versions.set('admin', version_1.Version.parse('3.4.5'));
            const javaAuthVersions = new versions_manifest_1.VersionsManifest({
                versionsMap: versions,
                version: version_1.Version.parse('1.2.3'),
            });
        ...
        (0, mocha_1.it)('updates versions.txt with an artifact id is another ones suffix without snapshot', async () => {
            const oldContent = (0, fs_1.readFileSync)((0, path_1.resolve)(fixturesPath, './versions-suffix-without-snapshot.txt'), 'utf8').replace(/\r\n/g, '\n');
            const versions = new Map();
            versions.set('google-cloud-admin', version_1.Version.parse('2.3.6'));
            versions.set('admin', version_1.Version.parse('3.4.3'));
            const javaAuthVersions = new versions_manifest_1.VersionsManifest({
                versionsMap: versions,
                version: version_1.Version.parse('1.2.3'),
            });
