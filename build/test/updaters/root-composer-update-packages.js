...
(0, mocha_1.describe)('composer-update-package.json', () => {
    (0, mocha_1.describe)('updateContent', () => {
        ...
        (0, mocha_1.it)('updates version in composer if it exists', async () => {
            const composer = new root_composer_update_packages_1.RootComposerUpdatePackages({
                version: version_1.Version.parse('1.0.0'),
            });
            const newContent = composer.updateContent('{"version": "0.8.0"}');
            (0, chai_1.expect)(newContent).to.eql('{"version":"1.0.0"}');
            snapshot(newContent);
        });
        (0, mocha_1.it)('does not update version in composer if it does not exist', async () => {
            const composer = new root_composer_update_packages_1.RootComposerUpdatePackages({
                version: version_1.Version.parse('1.0.0'),
            });
            const newContent = composer.updateContent('{}');
            (0, chai_1.expect)(newContent).to.eql('{}');
            snapshot(newContent);
        });
    });
});
